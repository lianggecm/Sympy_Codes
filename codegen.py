...
class CodeGen(object):
    ...
    def routine(self, name, expr, argument_sequence, global_vars):
        ...
        special_vars=expressions.atoms(Idx, Indexed, MatrixSymbol)
        idx_vars=[]
        indexed_vars=[]
        matrixsymbol_vars=[]
        for each in special_vars:
            if isinstance(each, Idx):
                idx_vars.append(each)
            elif isinstance(each, Indexed):
                indexed_vars.append(each)
            else:
                matrixsymbol_vars.append(each)

        # local variables
        local_vars = {i.label if isinstance(i, Idx) for i in Idx_vars}
        ...
        array_symbols = {}
        for array in indexed_vars:
            array_symbols[array.base.label] = array
        for array in matrixsymbol_vars:
